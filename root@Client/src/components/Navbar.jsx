...
const handleLogout= async()=>{
  ...
  toast.success(`You've been Logged out sucessfully !`, {
    duration: 1500,
  }); setLoading(false);
}
    return (
    <>
      <nav class="relative px-4 py-4 flex justify-between items-center h-full w-full bg-indigo-400 rounded-md rounded-t-none bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 border border-t-0 border-gray-100">
        <a 
                  onClick={()=>{nav('/')}}
        ...
        <ul class="hidden absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto lg:flex lg:items-center lg:w-auto lg:space-x-6">
          <li>
            <a class="font-extrabold text-transparent text-5xl bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600" href="#">
Quiz-O            </a>
          </li>
       ...
       {user ?<Dropdown     classNames={"py-2 top-8 -left-[220px] z-50 absolute w-max"}  button={
            <img
              className="h-10 w-10 ring-2 ring-gray-300 dark:ring-gray-500 bg-black rounded-full"
              src={'https://png.pngtree.com/png-clipart/20221207/ourmid/pngtree-fashion-boy-avatar-png-image_6514592.png'}
              alt="Elon Musk"
            />
          }>
               ...
               {user.isAdmin &&  <a onClick={()=>{nav('/Admin')}}
                ...
                <button disabled={loading}
                 onClick={handleLogout}
                  className="pt-2 mt-3 text-sm cursor-pointer border-t  font-medium text-red-500 hover:text-red-500"
                >
                {
                    loading?<div className="flex gap-2">   <div class="animate-spin ease-linear rounded-full w-5 h-5 border-t-2 border-b-2 border-blue-500 ml-3"></div>Logging you out...</div>:"Log Out"
                  }
                </button>
              </div>
            </div>
       </Dropdown>: 
        <Button extra={'hidden lg:inline-block '}  onClick={toggleSignup} title={"Sign Up"}> 
      </Button>}
      </nav>
